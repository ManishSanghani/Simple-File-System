...
/****************************************************************************/
/* returns the string representation of num in sō
...
void rm(char *name)
{
	...
	{ // If the directory entry is found
		...
		{
			int dblocks[3];
			_directory_entry dir_entries[4];
			int used_entries = 0;

			// Read the 3 block numbers from the inode of the directory to delete
			dblocks[0] = stoi(_inode_table[e_inode].XX, 2);
			dblocks[1] = stoi(_inode_table[e_inode].YY, 2);
			dblocks[2] = stoi(_inode_table[e_inode].ZZ, 2);

			// Loop through each of the 3 blocks
			for (int b = 0; b < 3; b++)
			{
				if (dblocks[b] == 0)
					continue;

				readSFS(dblocks[b], (char *)dir_entries);

				// Check each of the 4 entries in the block
				for (int k = 0; k < 4; k++)
				{
					if (dir_entries[k].F == '1')
					{
						used_entries++;
						break; // No need to check further, directory is not empty
					}
				}
			...
			if (used_entries > 0)
			{
				printf("%.252s: Directory is not empty.\n", name);
				return;
			}
	...
	while (1)
	{
		...
		*strchr(cmdline, '\n') = '\0';
		...
		else if (strcmp(tokens[0], "help") == 0)
		{
			...
			printf("  ls, rd, stats, exit\n");
			printf("  md <dir>   - Make directory\n");
			printf("  cd <dir>   - Change directory\n");
			printf("  create <file> - Create file\n");
			printf("  display <file> - Show file content\n");
